app.service('loginService',['$http', '$cookies', '$rootScope', 'clientesFactory',function($http, $cookies, $rootScope, clientesFactory)
{
            ...
            clientesFactory.confirmarEmail(email)
                .then(function (user) {
                    ...
                    if (user !== null && user.length > 0 && user[0].password === password) {
                        response = { success: true , userRole: user[0].role, nombre: user[0].nombre};
                    } else {
        ...
        this.SetCredentials = function(email, password, role, nombre) {
